...
define('MTS_TYPOGRAPHY_DEFAULT_OPT', 'google_typography_default');
define('MTS_TYPOGRAPHY_COLLECTIONS_OPT', 'google_typography_collections');
	...
	*/
	function _validate_options($plugin_options){
        ...
        if (!empty($plugin_options['translations']) && $plugin_options['translate'] == '1') {
            update_option('mts_translations_'.'spike', $plugin_options['translations']);
        }
...
echo '<span class="headtext">Welcome to your theme\'s mission control center.</span>';
					...
					echo '<a href="http://community.mythemeshop.com/" class="docsupport" target="_blank"><i class="icon-medkit"></i> Support</a>';
				...
				echo '<div id="nhp-opts-sidebar">';
					echo '<ul id="nhp-opts-group-menu">';
						foreach($this->sections as $k => $section){
							$icon = (!isset($section['icon']))?'<i class="icon-cogs"></i> ':'<i class="'.$section['icon'].'"></i> <br/>';
						...
						if(true === $this->args['show_typography']){
							echo '<li id="typography_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="typography_default_section_group_li_a" class="nhp-opts-group-tab-link-a" data-rel="typography_default"><i class="icon-text-width"></i><br/> '.'<span class="section_title">'.__('Typography', 'nhp-opts').'</span></a>';
							echo '</li>';
                        ...
                        if(true === $this->args['show_translate']){
							echo '<li id="translation_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="translation_default_section_group_li_a" class="nhp-opts-group-tab-link-a" data-rel="translation_default"><i class="icon-flag-alt"></i><br/> '.'<span class="section_title">'.__('Translate', 'nhp-opts').'</span></a>';
							echo '</li>';
						...
						if(true === $this->args['show_import_export']){
							echo '<li id="import_export_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="nhp-opts-group-tab-link-a" data-rel="import_export_default"><i class="icon-signin"></i><br/> '.'<span class="section_title">'.__('Import / Export', 'nhp-opts').'</span></a>';
							echo '</li>';
				...
				echo '<div id="nhp-opts-main">';
					...
					if(true === $this->args['show_import_export']){
						echo '<div id="import_export_default_section_group'.'" class="nhp-opts-group-tab">';
							...
							// import from link function removed
...
//							echo '</div>';
							...
							echo '</div>';
								...
								echo '<p><a href="javascript:void(0);" id="nhp-opts-export-code-copy" class="button-secondary">Show Export Data</a></p>';
								$backup_options = $this->options;
                                // google typography
                                $typography = get_option('google_typography_collections');
                                if (!empty($typography)) {
                                    $backup_options['google_typography_collections'] = $typography;
                                }
                                
                                $mts_translations = get_option('mts_translations_'.'spike');
                                if (!empty($mts_translations)) {
					...
					if(true === $this->args['show_translate']){
						echo '<div id="translation_default_section_group'.'" class="nhp-opts-group-tab">';
                            ...
                            echo '<div class="nhp-opts-section-desc">';
								echo '<p class="description">'. __('The theme translation panel provides an easy way to translate text appearing on your site, wthout the hassle of handling .po .mo files.', 'nhp-opts').'</p>';
	...
	*/
    ...
    function ajax_mts_translation_panel() {
        ...
        $i = 0;
        $mts_translations = get_option('mts_translations_'.'spike');//$this->options['translations'];
        foreach ($entries as $string_id => $object) {
            ...
            $value = (empty($mts_translations[$message]) ? $message : $mts_translations[$message]);
            echo '<div class="translate-string-wrapper">';
            // debug
            //echo '<!-- '.print_r($object,1).' -->';
            echo '<label for="translate-string-'.$i.'"><span>'.esc_html($message).'</span><input type="text" name="'.$this->args['opt_name'].'[translations]['._wp_specialchars( $message, ENT_QUOTES, false, true ).']" id="translate-string-'.$i.'" value="'._wp_specialchars( $value, ENT_QUOTES, false, true ).'"></label>';
            echo '</div>';
        }

...
class single_category_posts_widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		echo self::get_cat_posts( $cat, $qty, $comment_num, $date, $show_thumb1, $show_excerpt, $excerpt_length, $thumb_size );
		echo $after_widget;
	}

	public function get_cat_posts( $cat, $qty, $comment_num, $date, $show_thumb1, $show_excerpt, $excerpt_length, $thumb_size ) {
		$posts = new WP_Query(
			"cat=".$cat."&orderby=date&order=DESC&posts_per_page=".$qty
		);
        $class = ($thumb_size == 'widgetthumb' ? '' : ' bigthumbs');
		echo '<ul class="category-posts'.$class.'">';

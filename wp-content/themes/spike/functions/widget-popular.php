...
class mts_poluar_posts_widget extends WP_Widget {
	...
	public function __construct() {
 	...
 	public function form( $instance ) {
        ...
        $thumb_size = isset( $instance[ 'thumb_size' ] ) ? $instance[ 'thumb_size' ] : 'widgetthumb';
		?>
		<p>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('thumb_size'); ?>"><?php _e('Thumbnails Size','mythemeshop'); ?>:</label> 
            <select id="<?php echo $this->get_field_id('thumb_size'); ?>" name="<?php echo $this->get_field_name('thumb_size'); ?>" style="width:100%;">
                <option value="widgetthumb" <?php selected('widgetthumb', $thumb_size); ?>><?php _e('Small Thumbnails','mythemeshop'); ?></option>
                <option value="widgetbigthumb" <?php selected('widgetbigthumb', $thumb_size); ?>><?php _e('Big Thumbnails','mythemeshop'); ?></option>
            </select>
        </p>
	...
	public function widget( $args, $instance ) {
		...
		echo self::get_popular_posts( $qty, $comment_num, $date, $days, $show_thumb3, $show_excerpt, $excerpt_length, $thumb_size );
		echo $after_widget;
	}

	public function get_popular_posts( $qty, $comment_num, $date, $days, $show_thumb3, $show_excerpt, $excerpt_length, $thumb_size ) {
		...
		if ( $days ) {
			...
			add_filter( 'posts_where', 'mts_popular_filter_where' );
		}
		//Create a new filtering function that will add our where clause to the query
        if (!function_exists('mts_popular_filter_where')) {
    		function mts_popular_filter_where( $where = '' ) {
    		  global $popular_days;
		...
		<li>
			<a href="<?php the_permalink(); ?>">
				<?php if ( $show_thumb3 == 1 ) : ?>
					<?php if($thumb_size == "widgetbigthumb") { ?>
                        <?php the_post_thumbnail('widgetbigthumb',array('title' => '','class' => 'big-thumb')); ?>
	                    <?php } else { ?>
